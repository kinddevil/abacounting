...
use crate::middlewares::auauth;

#[tokio::main]
async fn main() {
  tracing_subscriber::registry()
    .with(
      tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| "example_todos=debug,tower_http=debug".into()),
    )
  ...
  let app = Router::new()
    ...
    .layer(
      ServiceBuilder::new()
        ...
        .layer(middleware::from_fn(auauth::auth))
        .into_inner(),
    )
    .with_state(db);
...
async fn todos_index(pagination: Option<Query<Pagination>>, State(db): State<Db>) -> impl IntoResponse {
